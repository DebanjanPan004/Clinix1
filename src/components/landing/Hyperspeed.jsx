...
const nsin = (value) => Math.sin(value) * 0.5 + 0.5
...
const DISTORTIONS = {
  ...
  turbulentDistortion: {
    ...
    getJS: (progress, time) => {
      ...
      const getX = (value) =>
        Math.cos(Math.PI * value * uFreq.x + time) * uAmp.x +
        Math.pow(Math.cos(Math.PI * value * uFreq.y + time * (uFreq.y / uFreq.x)), 2) * uAmp.y
      const getY = (value) =>
        -nsin(Math.PI * value * uFreq.z + time) * uAmp.z -
        Math.pow(nsin(Math.PI * value * uFreq.w + time / (uFreq.z / uFreq.w)), 5) * uAmp.w
      const distortion = new THREE.Vector3(
  ...
  deepDistortion: {
    ...
    getJS: (progress, time) => {
      ...
      const getX = (value) => Math.sin(value * Math.PI * uFreq.x + time) * uAmp.x
      const getY = (value) => Math.pow(value * uPowY.x, uPowY.y) + Math.sin(value * Math.PI * uFreq.y + time) * uAmp.y
      const distortion = new THREE.Vector3(
...
function interpolate(current, target, speed = 0.1, limit = 0.001) {
...
class CarLights {
  ...
  init() {
    ...
    if (Array.isArray(palette)) {
      ...
      palette = new THREE.Color(palette)
    }

    for (let index = 0; index < options.lightPairsPerRoadWay; index += 1) {
...
class LightsSticks {
  ...
  init() {
    ...
    let palette = options.colors.sticks
    if (Array.isArray(palette)) {
      palette = palette.map((color) => new THREE.Color(color))
    } else {
      palette = new THREE.Color(palette)
    ...
    for (let index = 0; index < totalSticks; index += 1) {
...
class App {
  constructor(container, options = {}) {
    ...
    const initWidth = Math.max(1, container.offsetWidth)
    const initHeight = Math.max(1, container.offsetHeight)
    ...
    this.camera = new THREE.PerspectiveCamera(options.fov, initWidth / initHeight, 0.1, 10000)
  ...
  loadAssets() {
    ...
    return new Promise((resolve) => {
      ...
      searchImage.addEventListener('load', function onSearchLoad() {
        assets.smaa.search = this
        manager.itemEnd('smaa-search')
      })

      areaImage.addEventListener('load', function onAreaLoad() {
  ...
  onMouseDown(event) {
    this.options.onSpeedUp?.(event)
    this.fovTarget = this.options.fovSpeedUp
    this.speedUpTarget = this.options.speedUp
  }

  onMouseUp(event) {
    this.options.onSlowDown?.(event)
  ...
  update(delta) {
    ...
    this.speedUp += interpolate(this.speedUp, this.speedUpTarget, lerpPercentage, 0.00001)
    ...
    const fovChange = interpolate(this.camera.fov, this.fovTarget, lerpPercentage)
    if (fovChange !== 0) {
  ...
  tick() {
    ...
    if (resizeRendererToDisplaySize(this.renderer, this.setSize) && this.hasValidSize) {
...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  return <div className="h-full w-full" ref={hyperspeed} />
}
